...
pub fn runner() {
    println!("Day 03 Part 1:\t{}", part1(INPUT1));
    println!("Day 03 Part 2:\t{}", part2(INPUT1));
}

// 3x3 kernel
const KERNEL3X3: [[i32; 2]; 8] = [
    [-1, -1],
    [-1, 0],
    [-1, 1],
    [0, -1],
...
fn part1(input: &str) -> i32 {
    ...
    let line_count = input.lines().count() as i32;
    let char_count = input.lines().nth(0).unwrap().chars().count() as i32;
    let input_vec: Vec<Vec<char>> = input.lines().map(|line| line.chars().collect()).collect();

    for (i, _) in input_vec.iter().enumerate() {
        let mut current_number = String::new();
        let mut current_is_part_number = false;
        for j in 0..(char_count + 1) as usize {
            let mut near_special = false;
            for k in KERNEL3X3 {
                let (x, y) = (i as i32 + k[0], j as i32 + k[1]);
                if x < 0 || y < 0 || x >= line_count || y >= char_count {
                    continue;
                }
                let (x, y) = (x as usize, y as usize);
                near_special = near_special || SPECIALS.contains(&input_vec[x][y]);
            }

            if j < char_count as usize {
                let c = input_vec[i][j];
                if c.is_digit(10) {
                    current_number.push(c);
                    current_is_part_number = current_is_part_number || near_special;
                    continue;
                }
            }
...
fn part2(input: &str) -> i32 {
    ...
    let mut part_number_lookup: HashMap<(usize, usize), i32> = HashMap::new();
    ...
    for (i, _) in input_vec.iter().enumerate() {
        ...
        for j in 0..(char_count + 1) as usize {
            ...
            let currrent_length = current_number.len();
            if currrent_length > 0 {
                if current_is_part_number {
                    let number = current_number.parse::<i32>().unwrap();

                    for index in 0..currrent_length {
                        let (x, y) = (i, (j - currrent_length) + index);
                        part_number_lookup.insert((x, y), number);
                    }
                }
                current_number = String::new();
                current_is_part_number = false;
    ...
    for (i, _) in input_vec.iter().enumerate() {
        for j in 0..(char_count as usize) {
            if input_vec[i][j] == '*' {
                let mut part_numbers: HashSet<i32> = HashSet::new();
                for k in KERNEL3X3 {
                    let (x, y) = (i as i32 + k[0], j as i32 + k[1]);
                    if x < 0 || y < 0 || x >= line_count || y >= char_count {
                        continue;
                    ...
                    let (x, y) = (x as usize, y as usize);
                    if let Some(number) = part_number_lookup.get(&(x, y)) {
                        part_numbers.insert(*number);
                    }
                }
                if part_numbers.len() == 2 {
                    let part_numbers_vec: Vec<i32> = part_numbers.into_iter().collect();
                    sum += part_numbers_vec[0] * part_numbers_vec[1];
                }
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_part1() {
        assert_eq!(part1(INPUT_TEST1), 4361);
        assert_eq!(part1(INPUT_TEST2), 4361);
    }

    #[test]
    fn test_part2() {
        assert_eq!(part2(INPUT_TEST1), 467835);
    }

    const INPUT_TEST1: &str = "467..114..
...*......
..35..633.
......#...
617*......
...
const INPUT1: &str = "...........441.................367................296........................................567..47.....45.................947.............
