...
fn part1(input: &str) -> u32 {
    ...
    for line in input.lines() {
        ...
        let first = output.chars().next().unwrap().to_digit(10).unwrap();
        let last = output.chars().last().unwrap().to_digit(10).unwrap();
...
fn replace_words_with_numbers(input: &str) -> String {
    ...
    for c in output.chars() {
        new_string.push(c);
        for (_, (key, rep)) in words.iter().enumerate() {
            if new_string.contains(key) {
...
fn part2(input: &str) -> u32 {
    ...
    for line in input.lines() {
        new_input.push_str(&replace_words_with_numbers(line));
        new_input.push('\n');
    }
...
mod tests {
    ...
    fn test_day01() {
        ...
        assert_eq!(part1(INPUT_TEST1), 142);
    }
